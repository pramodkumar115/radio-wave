...
class RadioTile extends ConsumerStatefulWidget {
  const RadioTile(
      ...
      required this.isAddedStream});
  final RadioStation? radio;
  final List<RadioStation> radioStations;
  final bool isAddedStream;
                ...
                ? Row(
                    ...
                    children: [
                        IconButton(
                            onPressed: () {
                              ref
                                  .read(addedStreamsDataProvider.notifier)
                                  .updateAddedStreams(widget.radioStations
                                      .where((r) =>
                                          r.stationUuid !=
                                          widget.radio!.stationUuid!)
                                      .toList());
                            },
                            icon:
                                FUI(RegularRounded.TRASH, color: Colors.black)),
                        IconButton(
                            onPressed: () {
                              showModalBottomSheet(
                                  ...
                                  builder: (context) => CreateEditStream(
                                      streams: widget.radioStations,
                                      selected: widget.radio));
                            },
                            icon: FUI(RegularRounded.EDIT, color: Colors.black))
                      ])
                : Container()
          ]),
        ));
